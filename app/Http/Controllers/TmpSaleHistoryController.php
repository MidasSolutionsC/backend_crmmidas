...
use App\Services\Implementation\TmpSaleHistoryService;
use App\Validator\TmpSaleHistoryValidator;

class TmpSaleHistoryController extends Controller{
  private $request;
  private $saleHistoryService;
  private $saleHistoryValidator;

  public function __construct(Request $request, TmpSaleHistoryService $saleHistoryService, TmpSaleHistoryValidator $saleHistoryValidator)
  {
  ...
  public function getFilterBySale($saleId){
    try{
      $result = $this->saleHistoryService->getFilterBySale($saleId);
      $response = $this->response();
